...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # Half
        half = device.type != 'cpu' and args.half # half precision only supported on CUDA
        if half:
            model.half()
    ...
    if not training:
        ...
        dataloader, dataset = create_dataloader(data, netParams, args.batch_size, valid=True, local_rank=args.local_rank, world_size=args.world_size)
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader,total=len(dataloader), desc=s)):
